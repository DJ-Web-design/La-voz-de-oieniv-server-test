...
access_type=offline&
redirect_uri=https%3A%2F%2Fwww.lavozdeoieniv.tk%2Fadmin&
response_type=code&
client_id=24315564955-nnq0cqp6e64khnq9h2g9p8asmnncei8e.apps.googleusercontent.com&
prompt=consent">Obtener Acceso</a>
		</template>
...
<script>
	import {imageDatabase} from "@/plugins/firebase.js";
	...
	export default {
		mounted(){
			if (this.$route.query && queryParse(this.$route.query)["@cc355_1D"] === "@mn1r1s_@1l3th") {
				this.authorized = true;
				this.access_token = queryParse(this.$route.query).access_token;
				this.refresh_token = queryParse(this.$route.query).refresh_token
			} else {
		...
		methods:{
			...
			recogerDatos() {
			    ...
			    if (fileTitle && fileDes && fileGal && this.file) {
			        ...
			        upload.on("state_changed", snapshot => {
			            ...
			            post("https://lavozdeoieniv.herokuapp.com/thumbCreate", data, config)
			            .then(res=>{
		    ...
		    enviarVideo() {
		        ...
		        post("https://lavozdeoieniv.herokuapp.com/upload-video", params, config)
		        .then(res=>{
		            console.log(res);
		        }).catch( err =>{
		            console.log(err);
